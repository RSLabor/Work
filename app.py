...
class SFTPDownloader:
    ...
    def _process_pending(self, client: paramiko.SSHClient, sftp: paramiko.SFTPClient, job_id: int):
        while not self._stop:
            tasks = self.db.list_tasks(['pending'], job_id=job_id)
            if not tasks:
                time.sleep(0.8)
                continue
            for t in tasks:
                ...
                if t['status'] == 'done':
                    continue
                self.db.update_task(task_id, status='downloading', last_error=None)
                if t.get('job_id'):
                ...
                try:
                    ...
                    if success:
                        self.db.update_task(task_id, status='done', retries=retries, last_error=None)
                        t['retries'] = retries
                    else:
                        self.db.update_task(task_id, status='error', last_error='timeout or failed', retries=retries)
                        if t.get('job_id'):
                            self.db.update_job(t['job_id'], last_error='timeout or failed')
                        t['retries'] = retries
                    ...
                    if t.get('job_id'):
                        self.db.update_job(t['job_id'], last_error=str(e))
